...
export const count = (target, input) => {
  return input.reduce((arr, curr) => arr.concat(curr), [])
...
export const totalSum = (input) => {
  const arr = input.flat().reduce((acc, curr) => {
    return acc + curr;
  }, 0);
  return arr;
};
...
export const findTallerThan = (minHeight, data) => {
  const tallerThan = data.filter(object => {
    return object.height > minHeight;
  });
  const nameArray = tallerThan.map(value => {
    return value.name;
  });
