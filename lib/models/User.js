...
//gets challenge counts for a user
schema.statics.getChallengeCounts = function(userId) {
  const Solution = this.model('Solution'); 
  const Challenge = this.model('Challenge');

  return Promise.all([
    Solution.getChallengesByUser(userId),
    Solution.getPassingSolutionsByCategory(userId),
    Challenge.find().count()
  ])
    .then(([
      challengesByUser, 
      passingSolutionsByCategory, totalNumberOfChallenges
    ]) => {
      const failed = challengesByUser
        .filter(challenge => challenge.solutions.every(solution => !solution.passed)).length;
      
      const passed = challengesByUser
        .filter(challenge => challenge.solutions.some(solution => solution.passed)).length;
      return {
        attempted: challengesByUser.length,
        failed,
        passed, 
        totalNumberOfChallenges,
        passingSolutionsByCategory
      };
    });
};
